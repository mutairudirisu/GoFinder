...
export default function LocationListingsPage({ params }: { params: Promise<{ location: string }> }) {
  ...
  const handleMarkerClick = (id: string) => {
    ...
    setTimeout(() => {
      const elements = document.querySelectorAll(`[data-listing-id="${id}"]`);
      elements.forEach(el => {
        ...
        if (parent instanceof HTMLElement && el instanceof HTMLElement) {
          const top = el.offsetTop - parent.offsetTop - 20;
          ...
          <AnimatePresence>
            {!activeListing || !isCollapsed ? (
              <motion.div
                ...
                className="fixed bottom-28 left-1/2 z-50 pointer-events-auto"
              >
                <button
                  onClick={() => {
                    if (isCollapsed) {
                      ...
                      // Smoothly drop the drawer to the lowest height
                  ...
                  className="flex items-center gap-2 px-6 py-3.5 rounded-full bg-[#222222] text-white font-bold text-sm shadow-2xl active:scale-95 hover:scale-105 transition-all duration-300"
                >
                  <span>{isCollapsed ? "List" : "Map"}</span>
                  <i className={`ph-bold ${isCollapsed ? "ph-list-bullets" : "ph-map-trifold"} text-base`} />
                </button>
              </motion.div>
            ) : null}
          </AnimatePresence>
