      ...
      <nav className={`flex-1 px-2 py-6 space-y-2 ${isCollapsed ? 'overflow-visible' : 'overflow-y-auto'}`}>
        ...
        <div className="relative group mb-4">
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-4 rounded-2xl bg-slate-900 text-white hover:bg-slate-800 transition-all text-left shadow-lg ${
              isCollapsed ? 'justify-center' : ''
            }`}
          >
            <i className="ph-bold ph-plus text-xl flex-shrink-0"></i>
            {!isCollapsed && <span className="font-bold">Create Listing</span>}
          </button>
          {isCollapsed && (
            <div className="absolute left-full ml-3 -mt-12 px-3 py-2 bg-slate-900 text-white text-xs font-medium rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 shadow-lg">
              Create Listing
            </div>
          )}
        </div>
