...
export default function UserSideNav() {
  ...
  const navItems = [
    { label: "Profile", icon: "ph-user", href: "/user/profile" },
    { label: "Bookings", icon: "ph-calendar", href: "/user/bookings" },
    { label: "Favorites", icon: "ph-heart", href: "/user/favorites" },
    { label: "Messages", icon: "ph-chats", href: "/user/messages" },
    { label: "Roommates", icon: "ph-users-three", href: "/user/roommates" },
    { label: "Notifications", icon: "ph-bell", href: "/user/notifications" },
    { label: "Settings", icon: "ph-gear", href: "/user/settings" },
  ];
        ...
        {!isCollapsed && (
          <Link href="/user/profile" className="flex items-center gap-2">
            <div className="w-8 h-8 bg-brand-500 rounded-full flex items-center justify-center">
              <i className="ph-bold ph-user text-white text-lg"></i>
            </div>
      ...
      <div className="border-t border-slate-200 p-2 space-y-2">
        ...
        <Link
          ...
          className={`px-4 py-3 bg-slate-50 hover:bg-brand-50 rounded-lg flex items-center gap-3 transition-colors cursor-pointer ${
            isCollapsed ? 'justify-center' : ''
          } ${isActiveLink('/user/profile') ? 'bg-brand-50 border border-brand-200' : ''}`}
        >
